...
impl Lang {
    ...
    pub fn alert_title(self, tool: &str) -> String {
        match self {
            Lang::Ko => format!("🛬 {tool} 경보"),
            Lang::En => format!("🛬 {tool} Alert"),
        }
    }
    /// 리셋 임박 제목 — 도구명 + 종류.
    pub fn reset_title(self, tool: &str) -> String {
        match self {
            Lang::Ko => format!("🛬 {tool} 리셋 임박"),
            Lang::En => format!("🛬 {tool} Reset Soon"),
        }
    }
    /// 잔여율 경보 본문 (도구명은 제목에 있으므로 상황만).
    pub fn alert_low(self, pct: f64) -> String {
        match self {
            Lang::Ko => format!("런웨이 {pct:.0}% 남음"),
            Lang::En => format!("{pct:.0}% remaining"),
        }
    }
    /// 예상 소진 경보 본문.
    pub fn alert_eta(self, mins: f64, pct: f64) -> String {
        match self {
            Lang::Ko => format!("약 {mins:.0}분 후 소진 ({pct:.0}% 남음)"),
            Lang::En => format!("runs out in ~{mins:.0} min ({pct:.0}% left)"),
        }
    }
    /// 리셋 임박 본문.
    pub fn alert_reset(self, mins: f64, pct: f64) -> String {
        match self {
            Lang::Ko => format!("약 {mins:.0}분 후 리셋 — {pct:.0}% 남음, 지금 더 써도 됩니다"),
            Lang::En => format!("resets in ~{mins:.0} min — {pct:.0}% left, use it now"),
        }
    }
}
