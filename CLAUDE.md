...
- **계정이 여럿일 때** (`claude_accounts.rs`) — 한 머신에 Claude 계정이 여러 개 있으면
  ...
  - **조직을 못 읽으면 기본 config dir(`~/.claude`) 계정을 활성으로 세운다.** 이 폴백이
    없으면 홈 config가 없거나 `oauthAccount`가 빈 환경에서 계정이 하나뿐인데도 대표가
    안 서고 카드가 통째로 빈다 — 계정 기능을 넣기 전에는 토큰만 있으면 동작했으므로
    그건 회귀다
  - **알려진 제약** — `CLAUDE_CONFIG_DIR`로 홈이 아닌 config dir에서 작업하면 카드
    헤더와 트레이가 기본 config dir 계정을 가리킨다. 시계열을 `~/.claude/projects`
    한 곳에서만 읽어 활성 판정이 그 디렉토리에 묶여 있기 때문이다. 계정 목록과 계정별
    잔여율은 정확하다. 이걸 풀려면 `collect_samples`가 config dir마다 따로 읽어야 한다
...
- **경보 키는 계정 수에 따라 `tool`과 `tool#id` 사이를 오간다** — 계정이 둘이었다가
  ...
  `prune_alert_keys`가 매 라운드 지나간 키를 걷어내되, **이번 라운드에 대상이 하나도
  없는 도구는 건드리지 않는다** — 429나 콜드 스타트로 잠깐 결측된 것뿐인데 키를 지우면
